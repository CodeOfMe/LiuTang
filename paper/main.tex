...
Among the JVM-based systems, Apache Flink \cite{carbone2015flink} pioneered the Dataflow Model's implementation with first-class support for event-time processing, watermarks \cite{akidau2015watermarks}, and exactly-once semantics via Chandy-Lamport checkpointing \cite{chandy1985distributed}. Its Python API (PyFlink) exposes these capabilities but requires a JDK installation and a Flink cluster---typically over 500 MB---creating the deployment and version-coupling issues that plague all JVM-backed Python APIs. Apache Spark Streaming \cite{zaharia2016spark} began with a micro-batch model (DStreams) and later added continuous processing. Structured Streaming \cite{armbrust2018structured} introduced event-time watermarks and stateful processing, but its Python API relies on Py4J and a Spark JVM cluster, and the micro-batch architecture inherently introduces latency. Apache Beam \cite{beam2016} provides a unified programming model across batch and streaming with runners for Flink, Spark, and Google Dataflow, but its Python SDK transpiles pipelines to a Java representation, requiring a JVM runner and creating SDK-to-runner version coupling. The common thread across all three is that Python developers cannot escape the JVM: the runtime demands a Java installation, a cluster infrastructure, and version alignment between Python bindings and Java internals.
...
At the architectural level, the Lambda Architecture \cite{marz2011} and the Kappa Architecture \cite{kreps2014} represent the two dominant paradigms for reconciling streaming and batch processing. Lambda separates computation into a batch layer for accurate, high-latency results and a speed layer for approximate, low-latency results, merging them in a serving layer---the well-known cost being the need to maintain two separate codebases for the same business logic. Kappa collapses this into a single stream processor backed by an immutable event log, enabling replay-based reprocessing instead of a separate batch path, but at the cost of requiring all processing logic to be expressible as a streaming operation. Neither paradigm provides a continuous mechanism for navigating between streaming and batch: Lambda forces a hard split, while Kappa forces everything into streaming. What is missing is a principled way to treat the streaming--batch spectrum as exactly that---a spectrum---and to control where on that spectrum a pipeline operates at any given moment, adapting in real time to workload conditions. This is precisely the gap that the viscosity-controllable architecture fills.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.9\columnwidth]{fig-viscosity-controller.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.9\columnwidth]{fig-window-types.pdf}
...
LiuTang provides five state primitives, each with optional TTL. ValueState stores a single value per key, with TTL checked on access so that if $\texttt{monotonic()} - \texttt{last\_access} > \texttt{ttl}$, the state is cleared. ListState maintains an append-only list per key. MapState provides a key-value map per partition key. ReducingState incrementally reduces values using a provided $\texttt{reduce\_fn}: \sigma \times \sigma \to \sigma$. AggregatingState accumulates values using an $\texttt{add\_fn}$ and optional $\texttt{merge\_fn}$. State is accessed through RuntimeContext, which provides per-key isolation via a nested dictionary structure: $\texttt{keyed\_states}[key][name] \to \texttt{State}$. Figure~\ref{fig:state-primitives} illustrates the five state primitives.

\begin{figure}[t]
\centering
\includegraphics[width=0.9\columnwidth]{fig-state-primitives.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.9\columnwidth]{fig-lambda-kappa-adaptive.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.9\columnwidth]{fig-delivery-semantics.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.9\columnwidth]{fig-radar-features.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.9\columnwidth]{fig-benchmark-throughput.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.9\columnwidth]{fig-radar-deployment.pdf}
